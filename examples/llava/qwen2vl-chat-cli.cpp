...
// [Previous function definitions remain unchanged: qwen2vl_eval_image_embed, eval_tokens, eval_id, eval_string, sample, etc.]
...
int main(int argc, char ** argv) {
    ...
    if (ctx_llava == NULL) {
        fprintf(stderr, "%s: error: failed to init llava context\n", __func__);
        llama_model_free(model);
        return 1;
    }

    // Persistent variables for conversation context
    int n_past = 0;
    int cur_pos_id = 0;
    struct common_sampler * smpl = common_sampler_init(ctx_llava->model, params.sampling);
    if (!smpl) {
    ...
    // fflush(stdout);
    std::cout << "Welcome to the interactive LLaVA chat!\n" << std::flush;
    std::cout << "Enter a prompt, or use '--image <path>' with an optional prompt before it (type 'exit' to quit):\n" << std::flush;
    std::cout << "Example: 'Describe this --image ./image.jpg'\n" << std::flush;
    std::cout << "> " << std::flush;
    // std::cout.flush();  // Flush C++ stream
    ...
    while (std::getline(std::cin, user_input)) {
        ...
        // Handle empty input
        if (user_input.empty()) {
            std::cout << "Please provide a prompt or '--image <path>'.\n> ";
            continue;
        }

        // Parse input for --image flag
        llava_image_embed * image_embed = nullptr;
        std::string prompt;
        std::string image_path;
        size_t image_flag_pos = user_input.find("--image");

        if (image_flag_pos != std::string::npos) {
            // Extract prompt (before --image) and image path (after --image)
            prompt = user_input.substr(0, image_flag_pos);
            std::string rest = user_input.substr(image_flag_pos + 7); // Skip "--image"
            std::istringstream iss(rest);
            iss >> image_path; // Get the first token after --image as the path
            ...
            if (image_path.empty()) {
                std::cout << "Error: No image path provided after '--image'. Try again.\n> ";
                continue;
            }

            // Load the image
            image_embed = llava_image_embed_make_with_filename(ctx_llava->ctx_clip, params.cpuparams.n_threads, image_path.c_str());
            if (!image_embed) {
                LOG_ERR("%s: failed to load image from %s\n", __func__, image_path.c_str());
                std::cout << "Failed to load image. Please check the path and try again.\n> ";
                continue;
            }

            // Use default prompt if none provided
            if (prompt.empty()) {
                prompt = params.prompt.empty() ? "describe the image in detail." : params.prompt;
            }
        } else if (prompt_contains_image(user_input)) {
            // Handle base64-encoded image in prompt
            prompt = user_input;
            image_embed = llava_image_embed_make_with_prompt_base64(ctx_llava->ctx_clip, params.cpuparams.n_threads, prompt);
            if (!image_embed) {
                LOG_ERR("%s: failed to load image from base64 prompt\n", __func__);
                std::cout << "Failed to load image from base64. Please try again.\n> ";
                continue;
            }
        ...
        // Process the prompt (with or without image)
        const int max_tgt_len = params.n_predict < 0 ? 256 : params.n_predict;

        // Construct system and user prompts
        std::string system_prompt = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n<|vision_start|>";
        std::string user_prompt = image_embed ? "<|vision_end>" + prompt + "<|im_end|>\n<|im_start|>assistant\n" : prompt + "<|im_end|>\n<|im_start|>assistant\n";

        // Evaluate system prompt and image (if present)
        eval_string(ctx_llava->ctx_llama, system_prompt.c_str(), params.n_batch, &n_past, &cur_pos_id, true);
        if (image_embed != nullptr) {
            auto image_size = clip_get_load_image_size(ctx_llava->ctx_clip);
            if (!qwen2vl_eval_image_embed(ctx_llava->ctx_llama, image_embed, params.n_batch, &n_past, &cur_pos_id, image_size)) {
                LOG_ERR("%s: failed to evaluate image embed\n", __func__);
                llava_image_embed_free(image_embed);
                std::cout << "Failed to process image. Please try again.\n> ";
                continue;
            }
        }
        eval_string(ctx_llava->ctx_llama, user_prompt.c_str(), params.n_batch, &n_past, &cur_pos_id, false);
    ...
    // Cleanup
    common_sampler_free(smpl);
    ctx_llava->model = NULL; // Avoid double-free since model is freed separately
    llava_free(ctx_llava);
    llama_model_free(model);
